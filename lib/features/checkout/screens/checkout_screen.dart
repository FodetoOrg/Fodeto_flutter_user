...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  double _calculateOriginalDeliveryCharge(
    ...
    // Min.charge + distance * chargeperkm
    // Min.charge + 2* chargeperkm + (distance-2)* max.charge
    if (store != null && distance != null) {
      if (distance <= 2) {
        deliveryCharge = minimumCharge + (distance * perKmCharge);
      } else {
        if(maximumCharge!= null){
          deliveryCharge = minimumCharge + ((distance - 2) * maximumCharge) + (2*perKmCharge);
        }else{
          deliveryCharge = minimumCharge + (distance * perKmCharge);
        }
      }
